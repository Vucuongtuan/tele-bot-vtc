...
export function renderNewsletter(folderName: string, articles: Article[], imageSources?: string[]): string {
  const cards = articles.map((article, index) => {
    ...
    return `
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;border-spacing:0;margin:0 0 24px;width:100%">
        <tr><td>
          <a href="${escapeHtml(article.url)}" style="text-decoration:none"><img src="${escapeHtml(imageSource)}" alt="${escapeHtml(article.title)}" width="520" style="display:block;width:100%;max-width:520px;height:auto;border:0"></a>
